...
window.countNQueensSolutions = function(n) {
  ...
  (function tryNext (col, majD, minD, row) {
    if (row === n) {
      count++;
      return;
    }
    var conflicts = col | majD | minD;
    var placement;
    for (var i = 0; i < n; i++){
      placement = 1 << i;
      if ((placement | conflicts) !== conflicts) {
        tryNext(placement | col, (placement | majD) >> 1, (placement | minD) << 1, row+1);
      }
    }
  })(0,0,0,0);
